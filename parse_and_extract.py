"""
GE HVAC Manuals RAG — Unified Script
=====================================
Combines batch ADE parsing + RAG vector store + Flask web UI.

Modes:
    python parse_and_extract.py                # launch web UI (default)
    python parse_and_extract.py --web          # launch web UI explicitly
    python parse_and_extract.py --terminal     # interactive terminal Q&A
    python parse_and_extract.py "question"     # one-shot CLI answer

    python parse_and_extract.py --parse        # parse 2 new files (validation)
    python parse_and_extract.py --parse --all  # parse all unprocessed files
    python parse_and_extract.py --parse --limit 10  # parse next N files

Parse output:
    parse_results/{category}/{stem}.txt
    chunk_images/{category}/{stem}/NNN_type_uuid.png

Categories (subdirs of ge_hvac_manuals/):
    connect_series
...
MAX_PAGES = 90  # ADE hard limit is 100; use 90 for safety
...
def _split_pdf(pdf_path: Path, chunk_size: int = MAX_PAGES) -> list[Path]:
    """Split a PDF into <chunk_size page temp files. Returns list of temp paths."""
...
def _parse_split(client, category: str, pdf_path: Path, total_pages: int,
    ...
    """Parse a large PDF by splitting into chunks and merging results."""
    ...
    try:
        for part_idx, (page_offset, tmp_path) in enumerate(parts):
            ...
            # Adjust chunk page numbers to reflect position in original PDF
            ...
            # Crop images using the SPLIT pdf but with original page numbers in filenames
            for i, chunk in enumerate(part_data.get("chunks", [])):
                grounding = chunk.get("grounding")
                if not grounding:
                    continue
                box      = grounding.get("box", {})
                orig_page = grounding.get("page", page_offset)  # already adjusted above
                _crop_single_chunk(tmp_path, chunk, i + total_chunks, orig_page - page_offset, images_dir)
    ...
    # Build merged result using a synthetic parse-like structure
    merged = {
...
def _crop_single_chunk(pdf_path: Path, chunk: dict, global_idx: int,
    ...
    """Crop and save a single chunk image from a split PDF part."""
    import fitz
    from PIL import Image
    ...
    try:
        ...
        scale = dpi / 72
        mat   = fitz.Matrix(scale, scale)
        pix   = doc[local_page].get_pixmap(matrix=mat)
...
def run_parse(parse_all: bool = False, limit: int = 2):
    ...
    pdfs = _collect_pdfs()
    if not pdfs:
        print(f"No PDFs found under {INPUT_DIR}. Run download_ge_hvac_manuals.py first.")
        sys.exit(1)

    pending = [
        (cat, p) for cat, p in pdfs
        if not (PARSE_RESULTS / cat / f"{p.stem}.txt").exists()
    ]
...
def _load_friendly_names() -> dict:
    """Load UUID → friendly name map from friendly_names.json (if present)."""
...
def _load_all_chunks() -> list[dict]:
    """Read every parse_results/**/*.txt and extract chunks + metadata."""
...
def build_vector_store(force: bool = False):
    ...
    force=True  → always rebuild from parse_results/ (use after parsing is complete)
    force=False → always use existing store if it exists; never auto-rebuild
    """
    import numpy as np
    ...
    # Deduplicate by text before saving
...
def _deduplicate_chunks(chunks: list, vectors) -> tuple:
    """Remove chunks with duplicate text, keeping the first occurrence.
    Returns (deduped_chunks, deduped_vectors, removed_ids)."""
    import numpy as np
    seen_text  = {}
    keep_idx   = []
    removed_ids = []
...
def _delete_orphaned_images(chunk_ids: list[str]) -> None:
    """Delete chunk image files for the given chunk IDs."""
    deleted = 0
...
def _build_chunk_image_map() -> dict[str, Path]:
    """Map chunk_id (UUID) → image Path across all chunk_images subdirs."""
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — SEARCH + CLAUDE GENERATION
# ══════════════════════════════════════════════════════════════════════════════
...
def run_web(embedder, vectors, store, image_map):
    ...
    def chunk_image(filename):
        # filename is relative from chunk_images/ e.g. annual_reports/stem/000_text_uuid.png
